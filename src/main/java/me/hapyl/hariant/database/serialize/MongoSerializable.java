...
public interface MongoSerializable {
    ...
    @NotNull
    static <E extends PlayerDatabaseEntry> E deserialize(
            @NotNull String parent,
            @NotNull Class<E> clazz,
            @NotNull PlayerDatabase playerDatabase,
            @NotNull Document root,
        ...
        try {
            ...
            final Constructor<E> constructor = clazz.getDeclaredConstructor(Holder.CONSTRUCTOR_PARAMETERS);
            ...
            throw HariantPlugin.severeExceptionShutdownServer(new RuntimeException(
                    "Error deserializing `%s`: Missing constructor `%s(%s)`!".formatted(simpleClassName, clazz.getSimpleName(), Holder.CONSTRUCTOR_PARAMETERS_NAMES)
            ));
        }
        catch (Exception ex) {
