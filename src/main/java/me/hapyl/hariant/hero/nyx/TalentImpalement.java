...
public final class TalentImpalement extends TalentUltimateOvercharge {
    ...
    @DisplayField private final AttributeScaling damage = AttributeScaling.of(AttributeType.ATTACK, 54);
    @DisplayField private final AttributeScaling damageOvercharged = AttributeScaling.of(AttributeType.ATTACK, 81);
    ...
    @DisplayField private final BoundingBoxBlueprint spearBoundingBox = BoundingBoxBlueprint.define(1.2, 5, 1.2);
    ...
    public class Impalement extends HariantTickingTask implements EntityCollector {
        ...
        public void summonSpear() {
            ...
            LocationHelper.offset(location, x, 0, z, () -> {
                final DisplayEntity display = model.spawn(locationFrom, self -> {
                    self.setTeleportDuration(2);
                });
                
                HariantTask.later(() -> display.teleport(locationTo), 2);
                
                player.delegate(HariantTask.remove(() -> display, 5));
            });
